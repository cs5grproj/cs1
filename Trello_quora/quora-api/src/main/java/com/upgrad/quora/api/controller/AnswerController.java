...
public class AnswerController {
  ...
  public ResponseEntity<AnswerResponse> createAnswer(
    ...
    // Declaring the object of AnswerEntity class and set its corresponding attributes using
    // answerRequest
    AnswerEntity answerEntity = new AnswerEntity();
    answerEntity.setAns(answerRequest.getAnswer());
    answerEntity.setUuid(UUID.randomUUID().toString());
    // After setting the attributes of answerEntity, pass it to createAnswer() method, to add this
    // object to a persistent context
    final AnswerEntity createdAnswer =
        answerService.createAnswer(questionId, authorization, answerEntity);
  ...
  public ResponseEntity<AnswerEditResponse> editAnswer(
    ...
    // After setting the attributes of answerEntity, pass it to editAnswer() method, to add this
    // object to a persistent context
    final AnswerEntity editedAnswer =
        answerService.editAnswer(answerId, authorization, answerEntity);
    ...
    AnswerEditResponse answerEditResponse =
        new AnswerEditResponse().id(editedAnswer.getUuid()).status("ANSWER EDITED");
  ...
  @RequestMapping(
      ...
      produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
  public ResponseEntity<AnswerDeleteResponse> editAnswer(
      @RequestHeader("authorization") final String authorization,
      @PathVariable("answerId") final String answerId)
      throws AuthorizationFailedException, AnswerNotFoundException {
    ...
    AnswerDeleteResponse answerDeleteResponse =
        new AnswerDeleteResponse().id(deleteAnswer.getUuid()).status("ANSWER DELETED");
  ...
  public ResponseEntity<List<AnswerDetailsResponse>> getAllAnswers(
      ...
      @PathVariable("questionId") final String questionId)
      throws AuthorizationFailedException, InvalidQuestionException {
    final List<AnswerEntity> receivedAllAnswers =
        answerService.getAllAnswers(questionId, authorization);
    ...
    for (AnswerEntity n : receivedAllAnswers) {
      ...
      answer
          .id(n.getUuid())
          .answerContent(n.getAns())
          .questionContent(n.getQuestion().getContent());
